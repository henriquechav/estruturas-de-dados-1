...
void inOrderAux(no *r, int *first)
{
    if (r == NULL) return;
    inOrderAux(r->esq, first);
    if (*first) {
        printf("%c", r->chave);
        *first = 0;
    } else {
...
void remover(arvoreb *tr, char chave)
{
    ...
    if (it != NULL) {
        if (it->dir == NULL && it->esq == NULL) { /* folha */
            ...
            subs = removerAntecessor(antecessor(it));
            subs->pai = aux;
            subs->esq = it->esq;
            subs->dir = it->dir;
